...
func TestExample(t *testing.T) {
	...
	test_structure.RunTestStage(t, "clone_sample_app", func() {
		repoName := "sample-node-app"
		repoURL := "https://github.com/robmorgan/sample-node-app.git"
		branchName := "master"
		...
		absFolderPath, err := filepath.Abs(repoName)
		require.NoError(t, err)
		...
		cmd := exec.Command("git", "clone", repoURL, "--branch", branchName, "--single-branch", destFolder)
		_, err = cmd.Output()
		require.NoError(t, err)
